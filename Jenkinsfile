...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="glass0815/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/glass0815/guestbook.git'
            }
        }
        stage('Build') {
            steps {
            ...
            steps{
                ...
                withSonarQubeEnv('SonarQube-Server'){
                    sh '''
                        ...
                        -Dsonar.host.url=http://43.201.116.148:9000 \
                        -Dsonar.login=1ca614b68fbb29020e59144970dfda5b9ed84c77
                    '''
                }
            }
        }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@3.34.146.32 sudo docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@3.34.146.32 sudo docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=43.201.87.67 \
                                        -e MYSQL_PORT=3306 \
